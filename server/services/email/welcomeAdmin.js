 ...
 */
const sendWelcomeEmail = async (email, password, name, department) => {
  try {
    const mailOptions = {
      from: `"Back2Campus" <${process.env.EMAIL_USER}>`,
      to: email,
      subject: "🔐 Welcome, Admin! Here are your login credentials.",
      html: `
      ...
      <p style="font-size: 16px; color: #4A5568; line-height: 1.6;">
        Hello ${name} admin of ${department} Department,
        <br/><br/>
      ...
      <div style="background-color: #EDF2F7; padding: 20px; margin: 24px 0; border-radius: 8px;">
        ...
        <div style="font-size: 16px; color: #2D3748; margin-bottom: 16px;">${email}</div>
